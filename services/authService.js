...
class AuthService {
    ...
    async registerUser(data) {
        const existingUser = await this.User.findOne({ $or: [{ email: data.email }, { username: data.username }] });
        if (existingUser) {
    ...
    async validateUser(data) {
        const existingUser = await this.User.findOne({ username: data.username });
        if (!existingUser) {
